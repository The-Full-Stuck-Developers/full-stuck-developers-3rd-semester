...
export function UserDeposit({ userId }: { userId: string }) {
    ...
    const handleSubmit = async () => {
        ...
        if (!num || num <= 0) return setStatus({ type: "error", message: "Enter valid amount" });
        ...
        try {
            ...
            const res = await fetch("http://localhost:5284/api/Transactions", {
                ...
                body: JSON.stringify({ userId, amount: num, mobilePayTransactionNumber: ref }),
            });
    ...
    return (
        <div>
            ...
            <div className="bg-gray-800 rounded-3xl border border-gray-700 shadow-2xl p-8 max-w-2xl">
                <div className="mb-8">
                    <label className="block text-sm font-bold text-gray-300 mb-3">Amount (kr)</label>
                    <input
                        type="number"
                        value={amount}
                ...
                {status && (
                    <div className={`mb-6 p-5 rounded-2xl border flex gap-4 ${
                        ...
                        <span className="text-2xl">{status.type === "success" ? "Success" : "Warning"}</span>
                        <div>
                ...
                <p className="mt-6 text-center text-sm text-gray-400">
                    70% to prizes • 30% supports <span className="font-bold text-red-400">Jerne IF</span>
                </p>
            </div>
        </div>
    );
