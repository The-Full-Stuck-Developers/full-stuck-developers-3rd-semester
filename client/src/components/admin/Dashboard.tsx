...
export default function Dashboard() {
    ...
    const [healthStatus, setHealthStatus] = useState<object>({app: null, db: null});

    const healthCheck = async () => {
        const client = getHealthClient();
        try {
            // Run both requests in parallel
            const [appRes, dbRes] = await Promise.all([client.up(), client.databaseUp()]);
    ...
    const fetchPendingTransactionsCount = () => {
        ...
        client.getPendingTransactionsCount()
            .then(res => {
        ...
        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 text-white">
                ...
                >
                    <div className="flex items-start justify-between mb-4">
                        ...
                        <div
                            className={`p-1 rounded-xl ${
                                healthStatus.app?.status === "up" ? "bg-green-900/30" : "bg-red-900/30"
                            }`}
                        >
                            <Clock
                    ...
                    <p className="text-xs mt-1 text-white">
                        Checked at: {new Date(healthStatus.app?.timestamp).toLocaleTimeString()}
                    </p>
                </div>
                ...
                >
                    <div className="flex items-start justify-between mb-4">
                        ...
                        <div
                            className={`p-1 rounded-xl ${
                                healthStatus.db?.status === "up" ? "bg-green-900/30" : "bg-red-900/30"
                            }`}
                        >
                            <Clock
                    ...
                    <p className="text-xs mt-1 text-white">
                        Checked at: {new Date(healthStatus.db?.timestamp).toLocaleTimeString()}
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 text-white">
                ...
                <div className="bg-gray-800 rounded-2xl p-6 border border-gray-700">
                    ...
                    <p className="text-sm text-gray-400">{ t('transactions_pending_approval') }</p>
                    {pendingTransactionsCount > 0 && (
                        <p className="text-xs mt-1 text-red-400 animate-pulse">{ t('requires_action') }</p>
                    )}
                </div>
