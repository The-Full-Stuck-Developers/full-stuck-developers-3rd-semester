...
import {type TransactionDto, TransactionsClient, TransactionStatus, type UserDto} from "@core/generated-client.ts";
...
export default function TransactionsList() {
    ...
    const buildFilterString = () => {
        ...
        if (userSearchFilter) {
            filters.push(`User.Name@=*${userSearchFilter}|User.Email@=*${userSearchFilter}`);
        }
    ...
    const fetchTransactions = (page: number) => {
        ...
        client.getAllTransactions(filterString, null, page, pageSize)
            .then(res => {
    ...
    const handleApproveTransaction = (transaction: TransactionDto) => {
        ...
        client.approveTransaction(transaction.id!)
            .then((res) => {
                console.log(res);
                fetchTransactions(currentPage);
    ...
    return (
        <div>
            ...
            <div className={"flex flex-row items-center justify-between w-full pb-8"}>
                <p className={"text-white text-3xl mb-2 mx-0 p-0"}>{t("transactions")}</p>
            </div>
                ...
                <div className={"flex flex-row gap-4"}>
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option
                                value={TransactionStatus.Pending}>{TransactionStatus[TransactionStatus.Pending]}</option>
                            <option
                                value={TransactionStatus.Accepted}>{TransactionStatus[TransactionStatus.Accepted]}</option>
                            <option
                                value={TransactionStatus.Rejected}>{TransactionStatus[TransactionStatus.Rejected]}</option>
                            <option
                                value={TransactionStatus.Cancelled}>{TransactionStatus[TransactionStatus.Cancelled]}</option>
                        </select>
                    </div>
                </div>
                <div>
                                ...
                                <tr key={transaction.id} className="">
                                    <td className={`px-3 py-3 ${isLast ? "rounded-bl-xl" : ""}`}>
                                        <div className="flex items-center gap-2">
                                            <div
                                                className={"w-8 h-8 min-w-8 min-h-8 rounded-full bg-gradient-to-br from-red-500 to-red-700 flex items-center justify-center text-white font-bold text-xs"}>
                                                {transaction.user?.name?.trim().split(/\s+/).map(w => w[0].toUpperCase()).join('') || 'U'}
                                            </div>
                                            <div className="flex flex-col">
                                                <span
                                                    className="text-sm font-medium">{transaction.user?.name || 'N/A'}</span>
                                        ...
                                        <ActionMenu
                                            ...
                                            actions={[
                                                ...
                                                // ...(!transaction.user ? [
                                                {
                                                    ...
                                                    onClick: () => navigate(`/admin/transactions/user/${transaction.user.id}`)
                                                },
                                                // ] : []),
                                            ]}
                                        />
