...
export default function GameDetails() {
    ...
    const [
        ...
        setUpdateGameWinningNumbersModalOpen,
    ] = useState(false);
    const [isDrawGameWinnersModalOpen, setDrawGameWinnersModalOpen] =
        useState(false);
    const [isCalculateWinningsModalOpen, setCalculateWinningsModalOpen] =
        useState(false);
    const [inPersonWinners, setInPersonWinners] =
        useState<number>(0);
    ...
    const weekRange = game ? getIsoWeekRange(game.year, game.weekNumber) : null;
    ...
    const handleUpdateGameWinningNumbers = () => {
        const client = getGameClient();
        const winningNumbersDto: WinningNumbersDto = {
            winningNumbers: selected.join(","),
        };

        client
            .updateWinningNumbers(game?.id!, winningNumbersDto)
            .then((res) => {
                fetchGame();
    ...
    const calculateWinningsPerPlayer = () => {
        ...
        const totalWinningPrizePool: number = (game?.revenue! * 0.7) + game?.inPersonPrizePool!;
            ...
            <div className={"flex flex-row items-center justify-between w-full pb-8"}>
                <div className={"flex flex-col items-baseline"}>
                    <p className={"text-white text-3xl mb-2 mx-0 p-0"}>
                        {t("week")} {game?.weekNumber} / {game?.year}
                    </p>
                    <p className="text-sm text-gray-400 mt-0 pt-0">
                        {weekRange &&
                            `${format(weekRange.start)} – ${format(weekRange.end)}`}
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 text-white">
                        ...
                        {game?.drawDate == null && (
                            <div>
                                <button
                                    ...
                                    className={
                                        "w-full mt-2 flex flex-row items-center justify-evenly px-3 py-1.5 rounded-lg bg-[#e30613] hover:bg-[#c20510] text-white shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer"
                                    }
                                >
                                    {game?.winningNumbers == null
                                        ? t("game:pick_now")
                                        : t("game:update_numbers")}
                                </button>
                            </div>
                        )}
                    </div>
                ...
                <div
                    ...
                    <div className={"w-full"}>
                        ...
                        <div>
                            Jerne IF: {game?.revenue * 0.3} dkk / {t("game:players")}:{" "}
                            {game?.revenue * 0.7} dkk
                        </div>
                    </div>
                    <p className="text-sm text-gray-400 mt-3">
                        {t("game:total_revenue")}
                    </p>
                </div>
            ...
            {(game?.bets?.length ?? 0 > 0) ? (
                <div>
                    <div className={"flex flex-row items-baseline pb-4 "}>
                        <p className={"text-3xl mx-0 p-0 text-white"}>
                            {t("game:online_bets")}
                        </p>
                    </div>
                    <div
                            ...
                            {game?.bets.map((bet, index) => {
                                ...
                                return (
                                    <tr key={bet.id} className="">
                                        ...
                                        <td className={"px-3 py-3"}>{bet.user?.email}</td>
                                        <td className={"px-3 py-3"}>{bet.selectedNumbers}</td>
                                        <td className={`px-3 py-3`}>{bet.transaction.amount}</td>
                                        <td
                                            className={`px-3 py-3 ${isLast ? "rounded-br-xl" : ""}`}
                                        >
                                            {bet.isWinning ? (
                                                <CheckCheck color={"#10ff00"}/>
                                            ) : (
                                                <X color={"#ff0000"}/>
                                            )}
                                        </td>
                        ...
                        <div className={"text-lg py-4"}>
                            <div>
                                ...
                                <input
                                    ...
                                    required={true}
                                    onChange={(e) =>
                                        setInPersonWinners(Number(e.target.value))
                                    }
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    required={true}
                                    onChange={(e) =>
                                        setInPersonPrizePool(Number(e.target.value))
                                    }
                                    className="w-full pl-3 py-2 bg-gray-700 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
                                />
                            </div>
